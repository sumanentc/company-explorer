...
def get_summary_prompt(name, user_input, context):
    return f"""
            - You role is to generate a summary of the company {name} based on the context.
...
def run(prompt: str, api_key: str, json_result=True, ):
    llm = ChatOpenAI(temperature=0, model_name='gpt-3.5-turbo', openai_api_key=api_key, max_retries=2,
                     max_tokens=500)
    messages = [
        SystemMessage(
...
def run_with_search(prompt, api_key):
    ...
    try:
        llm = ChatOpenAI(temperature=0, model_name="gpt-3.5-turbo", openai_api_key=api_key, streaming=True,
